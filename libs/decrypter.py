...
from libs import system_errors
from libs import winregistry, ntlm
from libs.dcerpc.v5 import transport, rrp, scmr, wkst, samr, epm, drsuapi
from libs.dcerpc.v5.dtypes import NULL
from libs.dcerpc.v5.rpcrt import RPC_C_AUTHN_LEVEL_PKT_PRIVACY, DCERPCException, RPC_C_AUTHN_GSS_NEGOTIATE
from libs.dcerpc.v5.dcom import wmi
from libs.dcerpc.v5.dcom.oaut import IID_IDispatch, IDispatch, DISPPARAMS, DISPATCH_PROPERTYGET, \
    VARIANT, VARENUM, DISPATCH_METHOD
from libs.dcerpc.v5.dcomrt import DCOMConnection, OBJREF, FLAGS_OBJREF_CUSTOM, OBJREF_CUSTOM, OBJREF_HANDLER, \
    OBJREF_EXTENDED, OBJREF_STANDARD, FLAGS_OBJREF_HANDLER, FLAGS_OBJREF_STANDARD, FLAGS_OBJREF_EXTENDED, \
    IRemUnknown2, INTERFACE
from libs.ese import ESENT_DB
from libs.dpapi import DPAPI_SYSTEM
from libs.smb3structs import FILE_READ_DATA, FILE_SHARE_READ
from libs.nt_errors import STATUS_MORE_ENTRIES
from libs.structure import Structure
from libs.structure import hexdump
from libs.uuid import string_to_bin
from libs.crypto import transformKey
from libs.krb5 import constants
from libs.krb5.crypto import string_to_key
try:
...
# Structure taken from mimikatz (@gentilkiwi) in the context of https://github.com/SecureAuthCorp/libs/issues/326
...
class NTDSHashes:
    ...
    def __decryptHash(self, record, prefixTable=None, outputFile=None):
        ...
        return answer
        print('Leaving NTDSHashes.__decryptHash')
    ...
    def dump(self):
        ...
        #print("Lets go printing that shit")
        while status == STATUS_MORE_ENTRIES:
            ...
            for user in resp['Buffer']['Buffer']:
                ...
                if crackedName['pmsgOut']['V1']['pResult']['cItems'] == 1:
                    ...
                    print('DRSCrackNames returned %d items for user %s, skipping' % (
                    crackedName['pmsgOut']['V1']['pResult']['cItems'], userName))
                try:
                    #print("decrypting things from ntds")
                    resultString = self.__decryptHash(userRecord,
